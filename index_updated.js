...
function addHilfskostenstelle() {
    ...
    updateDropdowns();
    addHilfskostenstelleButton.scrollIntoView({ behavior: 'smooth', block: 'center' });
}
...
function addEndkostenstelle() {
    ...
    updateDropdowns();
    addEndkostenstelleButton.scrollIntoView({ behavior: 'smooth', block: 'center' });
}
...
function displayTable(verrechnungArt, hilfskostenstellen, endkostenstellen) {
    ...
    kostenarten.forEach(kostenart => {
        ...
        endkostenstellen.forEach(ekst => {
            ...
            if (kostenart === "Primäre Kosten") {
                ...
                td.textContent = (isNaN(ekst.primaereGK) || isNaN(ekst.sekundaereGK) || ekst.primaereGK === null || ekst.sekundaereGK === null) ? "0.00" : (ekst.primaereGK + ekst.sekundaereGK).toFixed(2);
            }
            row.appendChild(td);
        });
    ...
    superContainer.setAttribute("class", "tableContainer");
    superContainer.appendChild(tableContainer);
    superContainer.appendChild(buttonsContainer);
